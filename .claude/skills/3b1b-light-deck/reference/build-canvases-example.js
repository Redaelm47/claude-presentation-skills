...
const OUT = path.join(__dirname, 'canvases-3b1b-light');
fs.mkdirSync(OUT, { recursive: true });

const C = { ink:'#000', sub:'#444444', muted:'#888888', blue:'#1C758A', yellow:'#E8C11C', teal:'#49A88F' };

const CSS = `
*{box-sizing:border-box;margin:0;padding:0}
html,body{width:1920px;height:1080px;overflow:hidden}
body{font-family:'CMU Serif',Georgia,serif;color:${C.ink};background:#fff;position:relative;
  display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}
.kick{font-size:26px;letter-spacing:7px;text-transform:uppercase;color:${C.muted}}
.corner{position:absolute;font-size:22px;letter-spacing:3px;color:#999}
...
function arcs() {
  ...
  for (let i = 0; i < n; i++) {
    ...
    s += hot
      ...
      : `<circle cx="${X(i)}" cy="${y}" r="9" fill="#fff" stroke="${C.blue}" stroke-width="3"/>`;
  }
  return s + '</svg>';
}
