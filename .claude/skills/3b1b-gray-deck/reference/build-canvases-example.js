...
const OUT = path.join(__dirname, 'canvases-3b1b-gray');
fs.mkdirSync(OUT, { recursive: true });

const C = { ink:'#fff', sub:'#BBBBBB', muted:'#9a9a9a', blue:'#58C4DD', yellow:'#FC6255', teal:'#5CD0B3' };

const CSS = `
*{box-sizing:border-box;margin:0;padding:0}
html,body{width:1920px;height:1080px;overflow:hidden}
body{font-family:'CMU Serif',Georgia,serif;color:${C.ink};background:#333333;position:relative;
  display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}
.kick{font-size:26px;letter-spacing:7px;text-transform:uppercase;color:${C.muted}}
.corner{position:absolute;font-size:22px;letter-spacing:3px;color:#8a8a8a}
...
function arcs() {
  ...
  for (let i = 0; i < n; i++) {
    ...
    s += hot
      ...
      : `<circle cx="${X(i)}" cy="${y}" r="9" fill="#333333" stroke="${C.blue}" stroke-width="3"/>`;
  }
  return s + '</svg>';
}
